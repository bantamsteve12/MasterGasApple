...
- (void)downloadDataForRegisteredObjects:(BOOL)useUpdatedAtDate toDeleteLocalRecords:(BOOL)toDelete {
    ...
    for (NSString *className in self.registeredClassesToSync) {
        ...
        AFHTTPRequestOperation *operation = [[SDAFParseAPIClient sharedClient] HTTPRequestOperationWithRequest:request success:^(AFHTTPRequestOperation *operation, id responseObject) {
                ...
                // SJL REMOVED NSLog(@"response object %@", responseObject);
...
- (void)processJSONDataRecordsForDeletion {
 ...
 // SJL REMOVED   [self postLocalObjectsToServer];
}
...
- (void)postLocalObjectsToServer {
    ...
    NSLog(@"posting Local objects on server");
    ...
    [[SDAFParseAPIClient sharedClient] enqueueBatchOfHTTPRequestOperations:operations progressBlock:^(NSUInteger numberOfCompletedOperations, NSUInteger totalNumberOfOperations) {
      ...
      // changed this to no longer call the delete objects method
    //SJL REMOVED    [self editLocalObjectsToServer];
        
    }];
}

// added this.
- (void)editLocalObjectsToServer {
   // NSLog(@"editLocalObjectsToServer");
    
    NSMutableArray *operations = [NSMutableArray array];
    for (NSString *className in self.registeredClassesToSync) {
        NSArray *objectsToEdit = [self managedObjectsForClass:className withSyncStatus:SDObjectEdited];
        for (NSManagedObject *objectToEdit in objectsToEdit) {
            NSDictionary *jsonString = [objectToEdit JSONToEditObjectOnServer];
       
            NSMutableURLRequest *request = [[SDAFParseAPIClient sharedClient] PUTRequestForClass:className parameters:jsonString forObjectWithId:[objectToEdit valueForKey:@"objectId"]];
            
        
            AFHTTPRequestOperation *operation = [[SDAFParseAPIClient sharedClient] HTTPRequestOperationWithRequest:request success:^(AFHTTPRequestOperation *operation, id responseObject) {
                
                NSDictionary *responseDictionary = responseObject;
                NSDate *updatedDate = [self dateUsingStringFromAPI:[responseDictionary valueForKey:@"updatedAt"]];
                
                [objectToEdit setValue:updatedDate forKey:@"updatedAt"];
                [objectToEdit setValue:[NSNumber numberWithInt:SDObjectSynced] forKey:@"syncStatus"];
                
            } failure:^(AFHTTPRequestOperation *operation, NSError *error) {
                NSLog(@"Failed edit: %@", error);
            }];
    ...
    [[SDAFParseAPIClient sharedClient] enqueueBatchOfHTTPRequestOperations:operations progressBlock:^(NSUInteger numberOfCompletedOperations, NSUInteger totalNumberOfOperations) {
        NSLog(@"Completed %d of %d edit operations", numberOfCompletedOperations, totalNumberOfOperations);
    } completionBlock:^(NSArray *operations) {
        if ([operations count] > 0) {
            NSLog(@"Edit of objects on server compelete, updated objects in context: %@", [[[SDCoreDataController sharedInstance] backgroundManagedObjectContext] updatedObjects]);
            [[SDCoreDataController sharedInstance] saveBackgroundContext];
            NSLog(@"SBC After call edit");
        }
     // SJL REMOVED   [self deleteObjectsOnServer];
    }];
}
...
-(void)imagesSync
{
    NSArray *objectsToCreate = [self managedObjectsForClass:@"Image" withSyncStatus:SDObjectCreated];
    for (NSManagedObject *imageObjectToCreate in objectsToCreate) {
        
        // get image name
        NSString *imageName = [imageObjectToCreate valueForKey:@"filename"];
        ...
        PFObject *imageObject = [PFObject objectWithClassName:@"Image"];
        [imageObject setObject:imageName forKey:@"filename"];
...
- (NSDictionary *)JSONDictionaryForClassWithName:(NSString *)className {
 ...
 //    NSURL *fileURL = [[NSBundle mainBundle] URLForResource:@"ApplianceType" withExtension:@"json"];
