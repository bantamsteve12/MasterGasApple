...
@implementation MileagePDFRenderer
...
NSMutableArray *mileageItemsArray;
...
+(void)drawPDF:(NSString*)fileName withMileageRecord:(Mileage *)mileage withMileageItems:(NSMutableArray *)mileageItems withCompanyRecord:(Company *)companyRecord
{
    ...
    mileageItemsArray = mileageItems;
    mile = mileage;
...
+(void)drawLabels
{
    ...
    NSArray* objects = [[NSBundle mainBundle] loadNibNamed:@"MileageReport" owner:nil options:nil];
        ...
        if([view isKindOfClass:[UILabel class]])
        {
            ...
            switch(label.tag)
            {
                ...
                case 10:{
                    ...
                    NSString *str = mile.engineerName;
                    if (str.length > 0) {
                        label.text = str;
                    }
                    else {
                ...
                case 30:{
                    ...
                    NSString *str = mile.uniqueClaimNo;
                    if (str.length > 0) {
                        label.text = str;
                    }
                    else {
                ...
                case 32:{
                 ...
                 //   [dateFormatter setTimeZone:[NSTimeZone timeZoneWithName:@"GMT"]];
                    ...
                    str = [dateFormatter stringFromDate:mile.date];
        ...
        andColumnCount:(int)numberOfColumns
{
    ...
    for(int i = 0; i < [mileageItemsArray count]; i++)
    {
        
        MileageItem *mileItem = [mileageItemsArray objectAtIndex:i];
        
        
        NSString *dateString = [dateFormatter stringFromDate:mileItem.date];
        
        
        NSArray* infoToDraw = [NSArray arrayWithObjects:
                               @"",
                               dateString,
                               [NSString checkForNilString:mileItem.journeyDescription],
                               [NSString checkForNilString:mileItem.journeyNotes],
                               [NSString checkForNilString:mileItem.vehicleRegistration],
                               [NSString checkForNilString:mileItem.startMileage],
                               [NSString checkForNilString:mileItem.finishMileage],
                               [NSString checkForNilString:mileItem.totalMileage],
                               nil];
...
+(void)calculateSummaryTotals
{
    ...
    if ([mileageItemsArray count] > 0) {
        ...
        for (int i = 0; i < [mileageItemsArray count]; ++i) {
            MileageItem *mileItem = [mileageItemsArray objectAtIndex:i];
            [totalArray addObject:[NSNumber numberWithInt:[mileItem.totalMileage intValue]]];
        }
