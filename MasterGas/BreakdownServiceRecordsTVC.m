...
@interface BreakdownServiceRecordsTVC ()
@property (nonatomic, strong) NSDateFormatter *dateFormatter;
@property (nonatomic, strong) NSManagedObjectContext *managedObjectContext;

@end

@implementation BreakdownServiceRecordsTVC
...
- (void)viewDidLoad
{
    NSLog(@"Breakdown service records: reportType = %@", self.recordType);
...
- (BOOL)shouldPerformSegueWithIdentifier:(NSString *)identifier sender:(id)sender {
    if ([identifier isEqualToString:@"AddMaintenanceServiceRecordSegue"]) {
        ...
        if (!segueShouldOccur) {
            UIAlertView *notPermitted = [[UIAlertView alloc]
                                         ...
                                         message:@"To add more Gas Breakdown / Service records you need to upgrade to the pro account."
                                         delegate:nil
...
-(void)setupTitleDescriptorsAndOrdering
{
    ...
    for (MaintenanceServiceRecord *cert in self.records)
    {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([self.entityName isEqualToString:@"MaintenanceServiceRecord"]) {
        ...
        cell.textLabel.text = maintenanceServiceRecord.uniqueSerialNumber;
        
        [self.dateFormatter setDateFormat:@"d MMM yyyy"];

    
       cell.textLabel.text = maintenanceServiceRecord.customerAddressName;
        
        [self.dateFormatter setDateFormat:@"d MMM yyyy"];
        NSString *detailLabel = [NSString stringWithFormat:@"%@ - %@", [self.dateFormatter stringFromDate:maintenanceServiceRecord.date], maintenanceServiceRecord.reference];
        
        NSString *siteAddress = [NSString stringWithFormat:@"%@, %@", [NSString checkForNilString:maintenanceServiceRecord.siteAddressLine1], [NSString checkForNilString:maintenanceServiceRecord.siteAddressPostcode]];
        cell.subtitleOneLabel.text = siteAddress;
        cell.subtitleTwoLabel.text = detailLabel;
        cell.subtitleThreeLabel.text = maintenanceServiceRecord.uniqueSerialNumber; */
        ...
        NSLog(@"cell: %@", maintenanceServiceRecord.uniqueSerialNumber);
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender {
      ...
      if ([segue.identifier isEqualToString:@"ShowMaintenanceServiceRecordSegue"]) {
        ...
        breakdownServiceRecordTVC.recordType = self.recordType;
        
        NSLog(@"record Type = %@", self.recordType);
        
        [breakdownServiceRecordTVC setUpdateCompletionBlock:^{
            [self loadRecordsFromCoreData];
            [self.tableView reloadData];
         //   [[SDSyncEngine sharedEngine] startSync];
        }];
