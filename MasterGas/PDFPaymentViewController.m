...
#import "PDFPaymentRenderer.h"


@interface PDFPaymentViewController ()

@end

@implementation PDFPaymentViewController

@synthesize webView;
@synthesize currentPayment;
...
-(void)generateCertificate
{
    ...
    aPayment = currentPayment;
    
    NSString* fileName = [self getPDFFileName];
    
    // Could probably tidy up in a future release to use type def
    NSString *certificateReportLayoutName = @"PaymentReceipt";
    
    
    Company *companyRecord = [self.companyRecords objectAtIndex:0];
    
    NSLog(@"companyName = %@", companyRecord.companyName);
    
    [PDFPaymentRenderer drawPDF:fileName withPayment:aPayment withInvoice:invoice withCompanyRecord:companyRecord withCertificateReportLayoutName:certificateReportLayoutName];
...
-(void)saveToDropbox
{
    ...
    NSString *filename = [NSString stringWithFormat:@"%@.pdf", currentPayment.paymentId];
...
-(void)emailToCustomer
{
    ...
    [picker setSubject:@"Payment Receipt attached"];
    
    // Set up recipients
   
    if ([invoice.customerEmail length] > 0) {
        NSArray *toRecipients = [NSArray arrayWithObject:invoice.customerEmail];
    ...
    [picker addAttachmentData:pdfData mimeType:@"application/pdf" fileName:[NSString stringWithFormat:@"%@.pdf",currentPayment.paymentId]];
    
    [picker setMessageBody:@"Please find attached your payment receipt." isHTML:NO];
