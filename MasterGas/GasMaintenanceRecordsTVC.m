...
@interface GasMaintenanceRecordsTVC ()
@property (nonatomic, strong) NSDateFormatter *dateFormatter;
@property (nonatomic, strong) NSManagedObjectContext *managedObjectContext;

@end

@implementation GasMaintenanceRecordsTVC
...
- (BOOL)shouldPerformSegueWithIdentifier:(NSString *)identifier sender:(id)sender {
    if ([identifier isEqualToString:@"AddRecordSegue"]) {
        ...
        if (!segueShouldOccur) {
            UIAlertView *notPermitted = [[UIAlertView alloc]
                                         ...
                                         message:@"To add more Gas Service Maintenance records you need to upgrade to the pro account."
                                         delegate:nil
...
-(void)setupTitleDescriptorsAndOrdering
{
    ...
    for (MaintenanceServiceCheck *cert in self.records)
    {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([self.entityName isEqualToString:@"MaintenanceServiceCheck"]) {
        ...
        cell.textLabel.text = maintenanceCheckRecord.customerAddressName;
        
        [self.dateFormatter setDateFormat:@"d MMM yyyy"];
        NSString *detailLabel = [NSString stringWithFormat:@"%@ - %@", [self.dateFormatter stringFromDate:maintenanceCheckRecord.date], maintenanceCheckRecord.reference];
        
        NSString *siteAddress = [NSString stringWithFormat:@"%@, %@", [NSString checkForNilString:maintenanceCheckRecord.siteAddressLine1], [NSString checkForNilString:maintenanceCheckRecord.siteAddressPostcode]];
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender {
    ...
    if ([segue.identifier isEqualToString:@"ShowRecordSegue"]) {
        ...
        GasMaintenanceChecklistHeaderTVC *gasMaintenanceChecklistHeaderTVC = segue.destinationViewController;
        gasMaintenanceChecklistHeaderTVC.recordPrefix = @"GSMR";
        gasMaintenanceChecklistHeaderTVC.recordType = self.recordType;
