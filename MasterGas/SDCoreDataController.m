...
#pragma mark - FILES
NSString *storeFilename = @"MasterGas.sqlite";
    ...
    if (useMigrationManager &&
      ...
      /*  _store = [_persistentStoreCoordinator addPersistentStoreWithType:NSSQLiteStoreType
        ...
        if (![_persistentStoreCoordinator addPersistentStoreWithType:NSSQLiteStoreType configuration:nil
                                                                 URL:[self storeURL] options:options error:&error]) {
...
- (BOOL)reloadStore {
    ...
    NSError *error = nil;
     ...
     NSURL *storeURL = [[self applicationDocumentsDirectory1] URLByAppendingPathComponent:@"MasterGas.sqlite"];
