...
- (void)loadRecordsFromCoreData {
    [self.managedObjectContext performBlockAndWait:^{
        ...
        if (self.customerId.length > 1) {
            [request setPredicate:[NSPredicate predicateWithFormat:@"(syncStatus != %d) AND (certificateType == %@) AND (customerId == %@)", SDObjectDeleted, self.certificateType, self.customerId]];
        }
        else
        {
           [request setPredicate:[NSPredicate predicateWithFormat:@"(syncStatus != %d) AND (certificateType == %@)", SDObjectDeleted, self.certificateType]];
        }
