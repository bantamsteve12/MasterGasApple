//
//  BreakdownServicingViewController.m
//  MasterGas
//
...
@interface BreakdownServicingViewController ()

@end

@implementation BreakdownServicingViewController

@synthesize webView;
@synthesize currentMaintenanceServiceRecord;
@synthesize maintenanceServiceItems;
...
-(void)generateCertificate
{
  ...
  //  NSMutableArray *applianceInpects = [NSMutableArray arrayWithArray:applianceInspections];
    
    
    // Could probably tidy up in a future release to use type def
    NSString *certificateReportLayoutName = @"BreakdownServiceRecord";
    

    [PDFMaintenanceRecordRenderer drawPDF:fileName withMaintenanceRecord:aMaintenanceServiceRecord withCertificateReportLayoutName:certificateReportLayoutName];
...
-(void)saveToDropbox
{
    ...
    NSString *filename = [NSString stringWithFormat:@"%@.pdf", currentMaintenanceServiceRecord.uniqueSerialNumber];
...
-(void)emailToCustomer
{
    ...
    [picker setSubject:@"Gas Breakdown / Servicing Record Attached"];
    
    // Set up recipients
    if ([aMaintenanceServiceRecord.customerEmail length] > 0) {
        NSArray *toRecipients = [NSArray arrayWithObject:aMaintenanceServiceRecord.customerEmail];
    ...
    [picker addAttachmentData:pdfData mimeType:@"application/pdf" fileName:[NSString stringWithFormat:@"%@.pdf",currentMaintenanceServiceRecord.uniqueSerialNumber]];
